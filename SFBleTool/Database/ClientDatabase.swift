...
extension SFClientDatabase {
    ...
    static func createAppTables() {
        ...
        guard let appDb = getAppDb() else {
            SFDatabaseLogger.error(port: .client, tag: logTag, step: .failure, type: .add, msgs: "appDb=nil")
            return
        }
        do {
            try appDb.create(table: BTUserModel.table, of: BTUserModel.self)
            SFDatabaseLogger.info(port: .client, tag: logTag, step: .success, type: .add, msgs: "")
        } catch let error {
...
extension SFClientDatabase {
    ...
    static func setActiveUser(_ user: BTUserModel) -> Bool {
        ...
        do {
            ...
            SFDatabaseLogger.error(port: .client, tag: logTag, step: .failure, type: .update, msgs: error.localizedDescription)
            return false
        }
    }
    ...
    static func getActiveUser() -> BTUserModel? {
        ...
        guard let appDb = getAppDb() else {
            SFDatabaseLogger.error(port: .client, tag: logTag, step: .failure, type: .find, msgs: "appDb=nil")
            return nil
        }
        do {
            let condition = BTUserModel.Properties.state.is(AccountState.active.rawValue)
            ...
            if let user = user {
                SFDatabaseLogger.info(port: .client, tag: logTag, step: .success, type: .find, msgs: "user=\(user)")
            } else {
                SFDatabaseLogger.info(port: .client, tag: logTag, step: .failure, type: .find, msgs: "user=nil")
            }
            return user
        } catch let error {
