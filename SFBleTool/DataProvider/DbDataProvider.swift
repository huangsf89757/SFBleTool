...
extension DbDataProvider: SFUserApi {
    func sendSmsCode(type: SFUser.SmsCodeType) async -> SFDataResponse {
        ...
        SFDsLogger.debug(port: .server ,tag: logTag, step: .start, msgs: "模拟耗时\(random)秒")
        do {
            try await Task.sleep(nanoseconds: UInt64(random) * 1_000_000_000)
        } catch {
            SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.failure), msgs: "模拟耗时被中断")
        ...
        SFDsLogger.debug(port: .server ,tag: logTag, step: .inProcess, msgs: "生成随机验证码", smsCode)
        self.smsCodes.append(smsCode)
        DispatchQueue.main.asyncAfter(deadline: .now() + 60) {
            self.smsCodes.removeAll { ele in
                ele == smsCode
            }
            SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.success), msgs: "验证码(\(smsCode))过期")
        }
        SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.success), msgs: smsCode)
    ...
    func signIn(phone: String, code: String) async -> SFDataResponse {
        ...
        guard let appDb = SFServerDatabase.getAppDb() else {
            SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.failure), msgs: "appDb=nil")
            return (code: .serverError, isSuccess: false, data: nil, message: nil)
        }
        ...
        guard let smsCode = smsCode else {
            SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.failure), msgs: "smsCode=nil")
        ...
        do {
            ...
            if var user = user {
                ...
                SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.success), msgs: user)
                return (code: .ok, isSuccess: true, data: user, message: nil)
            } else {
                SFDsLogger.debug(port: .server ,tag: logTag, step: .inProcess, msgs: "user=nil")
                ...
                SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.success), msgs: "注册用户", newUser)
                return (code: .ok, isSuccess: true, data: newUser, message: nil)
            }
        } catch let error {
            SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.failure), msgs: error.localizedDescription)
            return (code: .serverError, isSuccess: false, data: nil, message: nil)
        }
    }
    ...
    func signIn(account: String, pwd: String) async -> SFDataResponse {
        ...
        do {
            ...
            if var user = user {
                ...
                SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.success), msgs: "user=nil")
    ...
    func signOut() async -> SFDataResponse {
        ...
        guard let activeUser = UserModel.active else {
            SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.failure), msgs: "activeUser=nil")
            return (code: .badRequest, isSuccess: false, data: nil, message: nil)
        }
        guard let uid = activeUser.uid else {
            SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.failure), msgs: "uid=nil")
            return (code: .badRequest, isSuccess: false, data: nil, message: nil)
        }
        SFDsLogger.debug(port: .server ,tag: logTag, step: .inProcess, msgs: activeUser)
        guard let appDb = SFServerDatabase.getAppDb() else {
            SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.failure), msgs: "appDb=nil")
            return (code: .serverError, isSuccess: false, data: nil, message: nil)
        }
            ...
            if var user = user {
                ...
                for (key, value) in info {
                    if let mapping = BTUserModel.keyPathMapping[key] {
                        ...
                        SFDsLogger.debug(port: .server ,tag: logTag, step: .inProcess, msgs: "未知字段: \(key)")
                    }
                }
                try appDb.update(table: BTUserModel.table, on: properties, with: user, where: condition)
                SFDsLogger.debug(port: .server ,tag: logTag, step: .end(.success), msgs: user)
